...
//   currentSong.onerror = () =>
//     console.error("Error loading song:", currentSong.src);
//   if (!pause) {
//     currentSong
//       .play()
...
//     cardContainer.innerHTML = ""; // Clear the container before appending new content
...
//           if (!infoResponse.ok) {
//             console.error(
//               `Failed to fetch info.json for folder ${folder}: ${infoResponse.statusText}`
//             );
//             continue;
//           }
...
//       if (parts[1]) {
//         songs.push(decodeURIComponent(parts[1]));
//       }
//     }
//   }
...
//   // Add click listeners
//   Array.from(songListUL.querySelectorAll("li")).forEach((li) => {
...
async function getSongs(folder) {
  ...
  try {
    const infoResp = await fetch(`/${folder}/info.json`);
    const data = await infoResp.json();
    songs = data.songs.map(decodeURIComponent);
  } catch (err) {
...
// Display all album folders dynamically
// async function displayAlbums() {
...
//       let meta = { title: folder, description: "" };
//       try {
//         const infoResp = await fetch(`/songs/${folder}/info.json`);
//         meta = await infoResp.json();
//       } catch (err) {
...
//     }
//   });
const albumFolders = ["ncs", "pop", "rock","Love","cs","car","Just chill","old.music","punjabi","telgu",""]; // List all subfolders in songs/
...
async function displayAlbums() {
  ...
  // Load songs when album clicked
  setTimeout(() => {
    // slight delay to ensure all cards are in DOM
    Array.from(document.getElementsByClassName("card")).forEach((card) => {
      card.addEventListener("click", async () => {
